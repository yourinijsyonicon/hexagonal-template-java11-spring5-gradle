...
public class InMemoryCustomerRepository implements CustomerRepository, CustomersArchive {
    ...
    public void save(Customer customer) {
        customerMap.put(customer.takeSnapshot().getId(), customer.takeSnapshot());
    }
    ...
    public Optional<Customer> findByVatNr(String vatNr) {
        return customerMap.values()
                .stream()
                .filter(it -> it.getDto().getVatNr().equals(vatNr))
                .findFirst()
                .map(Customer::fromSnapshot);
    }
    ...
    private CustomerItem mapSnapshotToItem(CustomerId id, CustomerSnapshot snapshot) {
        return CustomerItem.newBuilder()
                ...
                .withDto(CustomerDTO.newBuilder()
                        .withCommercialName(snapshot.getDto().getCommercialName())
                        .withLegalName(snapshot.getDto().getLegalName())
                        .withContactPerson(snapshot.getDto().getContactPerson())
                        .withVatNr(snapshot.getDto().getVatNr())
                        .build())
                .build();
    }
}
