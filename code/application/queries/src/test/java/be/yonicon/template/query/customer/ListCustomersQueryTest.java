    ...
    class TestWhenFound {
        ...
        void itShouldGetCustomers() {
            assertThat(actuals)
                    .hasSize(2)
                    .anySatisfy(it -> {
                        assertThat(it.getId()).isEqualTo(expectedAmazon.takeSnapshot().getId());
                        assertThat(it.getDto()).isEqualTo(expectedAmazon.takeSnapshot().getDto());
                    })
                    .anySatisfy(it -> {
                        assertThat(it.getId()).isEqualTo(expectedAlibaba.takeSnapshot().getId());
                        assertThat(it.getDto()).isEqualTo(expectedAlibaba.takeSnapshot().getDto());
                    });
        }
    }
